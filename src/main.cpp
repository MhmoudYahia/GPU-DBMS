...
void testFilter()
{
    ...
    // Test filter with salary > 70000
    auto condition = ConditionBuilder::greaterThan("salary", "70000");
    Filter filterOp(testTable, *condition);

    // Execute on CPU
    Table resultCPU = filterOp.executeCPU();
    assert(resultCPU.getRowCount() == 3); // Rows with index 3, 4, 5 should satisfy this condition
    ...
    auto ageCondition = ConditionBuilder::greaterThan("age", "25");
    auto salaryCondition = ConditionBuilder::lessThan("salary", "90000");
    auto complexCondition = ConditionBuilder::And(std::move(ageCondition), std::move(salaryCondition));
    
    Filter complexFilterOp(testTable, *complexCondition);
    Table complexResult = complexFilterOp.executeCPU();
    
    // For the test data:
    // Person1: age=25, salary=60000 -> Doesn't match (age not > 25)
    // Person2: age=30, salary=70000 -> Matches
    // Person3: age=35, salary=80000 -> Matches
    // Person4: age=40, salary=90000 -> Doesn't match (salary not < 90000)
    // Person5: age=45, salary=100000 -> Doesn't match (salary not < 90000)
    assert(complexResult.getRowCount() == 2);
