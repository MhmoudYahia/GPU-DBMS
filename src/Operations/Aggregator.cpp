...
namespace GPUDBMS
{
    ...
    Table Aggregator::execute(bool useGPU)
    {
        if (useGPU)
        {
            return executeGPU();
                    ...
                    case DataType::INT:
                        ...
                        else
                        {
                            ...
                            case AggregateFunction::AVG:
                                resultTable.appendDoubleValue(colIndex,
                                                              values.empty() ? 0.0 : std::accumulate(values.begin(), values.end(), 0.0) / values.size());
                                break;
    ...
    Table Aggregator::executeGPU()
    {
        // Convert m_aggregations to the required format
        std::vector<std::pair<std::string, AggregateFunction>> aggregations;
        for (const auto &agg : m_aggregations)
        {
            aggregations.emplace_back(agg.columnName, agg.function);
        }

        // Handle group by columns (convert from optional<string> to vector<string>)
        std::vector<std::string> groupByColumns;
        if (m_groupByColumn.has_value())
        {
            groupByColumns.push_back(m_groupByColumn.value());
        }

        return launchAggregationKernel(m_inputTable, groupByColumns, aggregations, {});
    }
